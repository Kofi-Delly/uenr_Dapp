...
contract DappVotes {
  ...
  function createPoll(
    ...
    require(bytes(title).length > 0, 'Poll-Title field cannot be empty');
    require(bytes(description).length > 0, 'Poll-Description field cannot be empty');
    require(bytes(image).length > 0, 'Poll-Image URL field cannot be empty');
  ...
  function updatePoll(
    ...
    require(bytes(description).length > 0, 'Poll-Description field cannot be empty');
    require(bytes(image).length > 0, 'Poll-Image URL field cannot be empty');
    require(!polls[id].deleted, 'Poll already been deleted');
  ...
  function deletePoll(uint id) public {
    require(pollExist[id], 'Poll not found');
    require(polls[id].director == msg.sender, 'Unauthorized entity');
    require(polls[id].votes < 1, 'Poll has votes already, can nolonger delete this poll');
  ...
  function contest(uint id, string memory name, string memory image) public {
    require(pollExist[id], 'Poll not found');
    require(bytes(name).length > 0, 'Contestant name field cannot be empty');
    require(bytes(image).length > 0, 'Contestant image url field cannot be empty');
    require(polls[id].votes < 1, 'Voting has started in this poll already. You cannot contest anylonger');
    require(!contested[id][msg.sender], 'Already a Contestant. Cannot contest again');
